...
use trustgrant::{
    AuthorityId, CustomOperationName, EvaluationDenyReason, EvaluationEngine, EvaluationRequest,
    MintContext, RequestedCapability, RequestedOperation, ResourceBinding, ResourceContext,
    ResourceRef, TemplateRef, VerifiedRevocationState,
    discovery::{
        AuthorityKeyRecord, DelegatedPrincipalRef, ResolvedSignerBinding, SignatureProfile,
    },
    document::ValidatedTrustGrantDocument,
    document::raw::{
        RawMintingConstraints, RawSelector, RawSupersessionPolicy, RawTrustGrantDocument,
    },
    domain::{
...
fn parse_and_validate(json_str: &str) -> ValidatedTrustGrantDocument {
    ...
    ValidatedTrustGrantDocument::try_from(raw).unwrap_or_else(|e| panic!("validation failed: {e}"))
}
...
fn conformance_s2_5_revision_must_increase_monotonically() {
    ...
    let result = ValidatedTrustGrantDocument::try_from(
        RawTrustGrantDocument::parse_json_str(&json_zero)
            .unwrap_or_else(|e| panic!("raw parse: {e}")),
    );
...
fn conformance_s2_5_supersedes_must_be_same_series() {
    ...
    let json = make_grant_json(&[
        ...
        (
            "supersedes",
            json!("tg_00000000-0000-0000-0000-000000000000"),
        ),
    ]);
    let raw =
        RawTrustGrantDocument::parse_json_str(&json).unwrap_or_else(|e| panic!("raw parse: {e}"));
    let result = ValidatedTrustGrantDocument::try_from(raw);
    assert!(
        result.is_ok(),
        "revision 2 with valid supersedes should be accepted"
    );
    ...
    let json_self = make_grant_json(&[
        (
            "trustgrant_id",
            json!("tg_11111111-1111-4111-8111-111111111001"),
        ),
        ("revision", json!(2)),
        (
            "supersedes",
    ...
    let raw_self = RawTrustGrantDocument::parse_json_str(&json_self)
        .unwrap_or_else(|e| panic!("raw parse: {e}"));
...
fn conformance_s2_5_supersession_policy_valid_values() {
    ...
    assert_eq!(
        RawSupersessionPolicy::Coexist.as_str(),
        "coexist",
        "coexist policy should serialize to 'coexist'"
    );
    assert_eq!(
        RawSupersessionPolicy::SupersedePrevious.as_str(),
        "supersede_previous",
...
fn conformance_s4_required_fields_present_with_correct_types() {
    ...
    assert!(
        !raw.trustgrant_id.is_empty(),
        "trustgrant_id must be non-empty"
    );
    // grant_series_id is a string
    assert!(
        !raw.grant_series_id.is_empty(),
        "grant_series_id must be non-empty"
    );
    // version is integer
...
fn conformance_s4_missing_signature_rejected() {
    ...
    assert!(
        result.is_err(),
        "document without signature should be rejected"
    );
}
...
fn conformance_s4_revocation_endpoint_must_be_non_empty() {
    ...
    assert!(
        !revocation.revocation_endpoint.is_empty(),
        "revocation_endpoint must be non-empty"
    );
}
...
fn conformance_s5_target_scope_all_false_empty_allow_rejected() {
    ...
    assert!(
        result.is_err(),
        "all=false with empty allow should be rejected"
    );
}
...
fn conformance_s6_resource_type_key_non_empty_string() {
    ...
    assert!(
        raw.resource_scope
            .types
            .contains_key(&Utf16Key::new("item")),
...
fn conformance_s6_resource_scope_all_true_allow_null() {
    ...
    let rt = validated
        .resource_scope()
        .get(&trustgrant::domain::ResourceTypeName::new("item").unwrap())
        .unwrap();
    assert!(rt.all());
...
fn conformance_s6_1_operations_optional_null_is_v0_compat() {
    ...
    let rt = validated
        ...
        .unwrap();
    assert!(
        rt.operations().is_none(),
        "null operations should remain None"
    );
}
...
fn conformance_s6_1_reserved_names_rejected_as_custom() {
    ...
    assert!(CustomOpName::new("create").is_err(), "create is reserved");
    assert!(CustomOpName::new("mint").is_err(), "mint is reserved");
    assert!(
...
fn conformance_s6_1_explicit_operations_allow_primary_deny_subtractive() {
    ...
    let op2 = CustomOperationName::new("custom:op2").unwrap_or_else(|e| panic!("custom op2: {e}"));
    let mut resource2 =
        ResourceContext::new("item").unwrap_or_else(|e| panic!("resource context: {e}"));
    resource2
        .insert_selector("namespace", "weapons")
    ...
    assert!(
        outcome2.decision().is_allowed(),
        "custom:op2 should be allowed"
    );
...
fn conformance_s7_selector_all_true_with_expressions_rejected() {
    ...
    assert!(
        result.is_err(),
        "all=true with expressions should be rejected"
    );
}
...
fn conformance_s7_selector_all_false_both_empty_rejected() {
    ...
    assert!(
        result.is_err(),
        "all=false with both empty should be rejected"
    );
}
...
fn conformance_s7_expressions_can_be_null() {
    ...
    let sel = &validated.target_scope().allow()[0];
    assert!(
        sel.expressions().is_empty(),
        "null expressions should be empty"
    );
}
...
fn conformance_s9_multiple_audience_entries() {
    ...
    let request_c = make_recognize_request(
        "https://target.example.com",
        "https://audience-c.example.com",
        "weapons",
    );
    assert_eq!(
...
fn conformance_s10_allow_is_primary_and_explicit() {
    ...
    assert!(
        outcome.decision().is_allowed(),
        "matching allow should produce Allow"
    );
...
fn conformance_s12_minting_constraints_are_per_type() {
    ...
    .unwrap_or_else(|e| panic!("request: {e}"))
    .with_mint_context_for_testing(MintContext::new(4, 1)).verify_selectors();
    request
        .insert_audience_principal_selector("actor", "player-42")
        .unwrap_or_else(|e| panic!("principal selector: {e}"));
    ...
    // Exceed max_total
    let request_exceed_total = request.clone().with_mint_context_for_testing(MintContext::new(5, 1)).verify_selectors();
    ...
    // Exceed max_per_user
    let request_exceed_user = request.clone().with_mint_context_for_testing(MintContext::new(4, 2)).verify_selectors();
...
fn conformance_s12_max_per_user_must_be_non_negative() {
    ...
    let constraints = RawMintingConstraints::new(None, None);
    assert!(
        constraints.max_per_user.is_none(),
        "null max_per_user is valid"
    );
