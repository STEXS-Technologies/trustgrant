...
use trustgrant::{
    ...
    RevocationSourceKind, RevocationStatus, SignatureVerificationRequest, SignatureVerifier,
    TrustGrantError, VerificationMetadata, VerificationPipeline, VerificationPosture,
    VerifiedRevocationState, VerifiedTrustGrantRecord,
};
...
#[derive(Debug, Error)]
enum RehydrateVerifiedRecordError {
    #[error(transparent)]
...
fn run() -> Result<u64, RehydrateVerifiedRecordError> {
    ...
    let artifacts = pipeline.verify_json_str(VALID_TRUSTGRANT_JSON, &verifier, metadata)?;
    let record = VerifiedTrustGrantRecord::from(artifacts.verified_grant());

    let mut rehydrated_count = 0_u64;

    for _ in 0..iterations {
        let verified = black_box(&record).try_to_verified_grant()?;
        black_box(verified);
        rehydrated_count = rehydrated_count.saturating_add(1);
    }
...
fn parse_iterations(
...
) -> Result<u64, RehydrateVerifiedRecordError> {
    ...
    argument
        .parse::<u64>()
        .map_err(RehydrateVerifiedRecordError::InvalidIterations)
}

fn verification_metadata() -> Result<VerificationMetadata, RehydrateVerifiedRecordError> {
...
fn signer_binding() -> Result<ResolvedSignerBinding, RehydrateVerifiedRecordError> {
    Ok(ResolvedSignerBinding::new(
        ...
        trustgrant::SignatureProfile::new("jcs+ed25519", "RFC8785")?,
        Some(DelegatedPrincipalRef::new("service", "issuer-worker")?),
    ))
}

fn ownership_record() -> Result<OwnershipVerificationRecord, RehydrateVerifiedRecordError> {
...
fn fixed_timestamp(
    ...
    minute: u32,
    second: u32,
) -> Result<chrono::DateTime<Utc>, RehydrateVerifiedRecordError> {
    Utc.with_ymd_and_hms(year, month, day, hour, minute, second)
        .single()
        .ok_or(RehydrateVerifiedRecordError::InvalidFixtureTimestamp)
}
