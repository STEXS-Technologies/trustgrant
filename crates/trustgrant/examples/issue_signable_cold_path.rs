...
#[derive(Debug, Error)]
enum IssueSignableColdPathError {
    #[error(transparent)]
...
fn run() -> Result<u64, IssueSignableColdPathError> {
    let iterations = parse_iterations(std::env::args().skip(1))?;
    let draft = trustgrant_draft()?;
    let mut total_bytes = 0_u64;

    for _ in 0..iterations {
        let document = black_box(black_box(&draft).signable_document()?);
        total_bytes = total_bytes
            .saturating_add(document.trustgrant_id.len() as u64)
            .saturating_add(document.grant_series_id.len() as u64)
            .saturating_add(document.key_id.len() as u64);
    }
...
fn parse_iterations(
...
) -> Result<u64, IssueSignableColdPathError> {
    ...
    argument
        .parse::<u64>()
        .map_err(IssueSignableColdPathError::InvalidIterations)
}

fn trustgrant_draft() -> Result<TrustGrantDraft, IssueSignableColdPathError> {
...
fn fixed_timestamp(
    ...
    minute: u32,
    second: u32,
) -> Result<chrono::DateTime<Utc>, IssueSignableColdPathError> {
    Utc.with_ymd_and_hms(year, month, day, hour, minute, second)
        .single()
        .ok_or(IssueSignableColdPathError::InvalidFixtureTimestamp)
}
